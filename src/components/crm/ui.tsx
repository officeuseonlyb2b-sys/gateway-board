...
export const fmtDate = (iso: string) =>
  iso ? new Date(iso.length > 10 ? iso : iso + "T00:00:00").toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : "—";

export const fmtShort = (iso: string) =>
  iso ? new Date(iso.length > 10 ? iso : iso + "T00:00:00").toLocaleDateString("en-GB", { day: "2-digit", month: "short" }) : "—";

export const fmtTime = (iso: string) =>
  iso ? new Date(iso).toLocaleString("en-GB", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: true }) : "—";
...
export function StageBadge({ stage, className }: { stage: Stage; className?: string }) {
  return (
    <span className={cn("inline-flex items-center rounded-md px-2 py-1 text-[11px] font-medium whitespace-nowrap", STAGE_STYLES[stage], className)}>
      {stage}
    </span>
  );
}
...
export function StatCard({
  icon, label, value, hint, tone = "blue", trend, valueClass,
}: {
  icon?: ReactNode; label: string; value: ReactNode; hint?: string;
  tone?: Tone; trend?: ReactNode; valueClass?: string;
}) {
  return (
    <div className="bg-card border border-border rounded-xl p-4 flex items-start gap-3">
      {icon && (
        <div className={cn("h-10 w-10 rounded-lg flex items-center justify-center shrink-0", TONES[tone])}>{icon}</div>
      )}
