...
import {
  Phone, PhoneOff, MessageCircle, Mail, CalendarPlus, CheckCircle2, RefreshCw, FileText,
  Trophy, XCircle, Activity,
} from "lucide-react";
import {
  completeFollowup, logCall, logLeadActivity, scheduleFollowup, setQueryPriority, setQueryStage,
  useCrmEvents,
} from "@/lib/crm/store";
...
export function PriorityBadge({ priority, className }: { priority?: string; className?: string }) {
  ...
  return (
    <span className={cn("inline-flex items-center rounded-md border px-2 py-0.5 text-[11px] font-medium", tone, className)}>
      {p}
    </span>
  );
}
...
export function LeadTimeline({ queryId, limit }: { queryId: string; limit?: number }) {
  ...
  const items = useMemo(() => {
    const list = events.filter((e) => e.query_id === queryId).slice().sort((a, b) => (a.at < b.at ? 1 : -1));
    ...
    <ol className="relative pl-5">
      ...
      {items.map((e) => (
        <li key={e.id} className="relative pb-4">
          <span className={cn("absolute -left-5 top-1.5 h-2.5 w-2.5 rounded-full", DOT[e.type] ?? "bg-slate-400")} />
...
function FollowupDialog({ query, trigger, complete }: { query: CrmQuery; trigger: React.ReactNode; complete?: boolean }) {
  ...
  const submit = () => {
    if (complete) {
      completeFollowup(query.query_id, note.trim() || undefined, new Date(due).toISOString(), actor);
      toast.success("Follow-up completed");
    } else {
      scheduleFollowup(query.query_id, new Date(due).toISOString(), note.trim() || undefined, actor);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label>Note</Label>
            <Textarea rows={3} value={note} onChange={(e) => setNote(e.target.value)} placeholder="What was discussed / planned?" />
          </div>
        </div>
        <DialogFooter>
...
export function LeadQuickActions({ query }: { query: CrmQuery }) {
  const actor = useActor();
  const q = query;
  const act = (fn: () => void, msg: string) => { fn(); toast.success(msg); };

  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2">
        <Button size="sm" variant="outline" onClick={() => act(() => logCall(q.query_id, true, undefined, actor), "Call logged as connected")}>
          <Phone className="h-3.5 w-3.5 mr-1" /> Call
        </Button>
        <Button size="sm" variant="outline" onClick={() => act(() => logCall(q.query_id, false, undefined, actor), "Call logged as not connected")}>
          <PhoneOff className="h-3.5 w-3.5 mr-1" /> No answer
        </Button>
        <Button size="sm" variant="outline" onClick={() => act(() => logLeadActivity(q.query_id, "whatsapp_sent", { by: actor, title: `WhatsApp sent by ${actor}`, detail: q.mobile || q.contact_person }), "WhatsApp logged")}>
          <MessageCircle className="h-3.5 w-3.5 mr-1" /> WhatsApp
        </Button>
        <Button size="sm" variant="outline" onClick={() => act(() => logLeadActivity(q.query_id, "email_sent", { by: actor, title: `Email sent by ${actor}`, detail: q.email || q.contact_person }), "Email logged")}>
          <Mail className="h-3.5 w-3.5 mr-1" /> Email
        </Button>
        <FollowupDialog
          query={q}
          trigger={<Button size="sm" variant="outline"><CalendarPlus className="h-3.5 w-3.5 mr-1" /> Follow-up</Button>}
        />
        <FollowupDialog
          complete
          query={q}
          trigger={<Button size="sm" variant="outline"><CheckCircle2 className="h-3.5 w-3.5 mr-1" /> Complete</Button>}
        />
        <Button size="sm" variant="outline" onClick={() => act(() => logLeadActivity(q.query_id, "customer_replied", { by: actor, title: `Customer replied (${actor})` }), "Reply logged")}>
          <RefreshCw className="h-3.5 w-3.5 mr-1" /> Replied
        </Button>
        <Button size="sm" variant="outline" onClick={() => act(() => logLeadActivity(q.query_id, "negotiation_started", { by: actor, title: `Negotiation started by ${actor}` }), "Negotiation logged")}>
      ...
      <div className="grid grid-cols-2 gap-2">
        <Button asChild size="sm" variant="secondary">
          <Link
            to="/costing"
            search={{ id: undefined }}
            onClick={() => logLeadActivity(q.query_id, "quotation_started", { by: actor, title: `Quotation started by ${actor}`, detail: q.query_id })}
          >
            <FileText className="h-3.5 w-3.5 mr-1" /> Quotation
          </Link>
        </Button>
        <NewTaskDialog queryId={q.query_id} trigger={<Button size="sm" variant="secondary">Create Task</Button>} />
      </div>
      ...
      <div className="space-y-2">
        ...
        <Select value={q.priority || "Medium"} onValueChange={(v) => { setQueryPriority(q.query_id, v, actor); toast.success(`Priority set to ${v}`); }}>
      ...
      <div className="grid grid-cols-2 gap-2 pt-1">
        <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700" onClick={() => act(() => setQueryStage(q.query_id, "Confirmed", actor), "Lead marked converted")}>
          <Trophy className="h-3.5 w-3.5 mr-1" /> Converted
        </Button>
        <Button size="sm" variant="destructive" onClick={() => act(() => setQueryStage(q.query_id, "Lost", actor), "Lead marked lost")}>
