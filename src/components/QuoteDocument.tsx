    ...
    <div className="quote-print-target bg-white text-[#222] mx-auto"
      ...
      <div className="flex items-start justify-between gap-6 pb-3" style={{ borderBottom: `2px solid ${TEAL}` }}>
        ...
        <div className="text-right">
          <div style={{ fontSize: 20, fontWeight: 800, color: TEAL, letterSpacing: 0.5 }}>INTERNAL QUOTATION RECORD</div>
          <div style={{ fontSize: 10, color: "#333", marginTop: 4 }}>
            <div><b>Quote No:</b> {q.display_number || `${q.quote_number}-V${q.version}`}</div>
          ...
          <tbody>
            {q.itinerary.map((d, i) => (
              <Fragment key={d.day_number}>
                <tr style={{ background: i % 2 ? "#f7f9fa" : "#fff" }}>
                  <Td>{d.day_number}</Td>
            ...
            {/* Legacy summary rows are retained only for older saved records. */}
            {(!q.scenarios || q.scenarios.length === 0) && <>
      ...
      {(!q.scenarios || q.scenarios.length === 0) && (q.is_group && q.group_rows && q.group_rows.length > 0 ? (
        <div style={{ marginTop: 14 }}>
      ...
      {/* SELECTED RATE SHEET — the exact rows approved by Sales are immutable. */}
      {q.rate_sheet_rows && q.rate_sheet_rows.length > 0 && (
        <div style={{ marginTop: 14 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: TEAL, marginBottom: 4 }}>
            LOCKED RATE SHEET — SELECTED PAX ROWS
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 9.5 }}>
            <thead><tr style={{ background: TEAL, color: "#fff" }}>
              <Th>Hotel Option</Th><Th>Vehicle</Th><Th align="right">Pax</Th>
              <Th align="right">Single</Th><Th align="right">Double</Th>
              <Th align="right">Triple</Th><Th align="right">Quad</Th>
            </tr></thead>
            <tbody>
              {q.rate_sheet_rows.map((row, index) => (
                <tr key={`${row.option_key}-${row.vehicle}-${row.pax}-${index}`} style={{ background: index % 2 ? "#f7f9fa" : "#fff" }}>
                  <Td>{row.option_label}</Td>
                  <Td>{row.vehicle}</Td>
                  <Td align="right">{row.pax}</Td>
                  <Td align="right">{row.single > 0 ? inr(row.single) : "—"}</Td>
                  <Td align="right">{row.double > 0 ? inr(row.double) : "—"}</Td>
                  <Td align="right">{row.triple > 0 ? inr(row.triple) : "—"}</Td>
                  <Td align="right">{row.quad > 0 ? inr(row.quad) : "—"}</Td>
                </tr>
              ))}
            </tbody>
          </table>
          <div style={{ fontSize: 8.5, color: "#666", marginTop: 4, fontStyle: "italic" }}>
            Values are per person and include the selected services, markup and applicable company GST.
          </div>
        </div>
      )}
