...
export function CostingSheet({ draft, set }: { draft: QuoteDraft; set?: SetDraft }) {
  ...
  // ⚠️ IMPORTANT: Replace these with the exact IDs generated by your buildLandPart for Hindi, English, and Language
  const GUIDE_LANG_IDS = ['hindi', 'english', 'language']; 
  ...
  const toggle = (bucket: Bucket, day: number, id: string, allIds: string[], allDays?: number[]) => {
    if (!set) return;
    ...
    daysToUpdate.forEach(dayNum => {
      const current = map[dayNum] ?? allIds;
      
      // Single Select for Guide Languages only (HINDI, ENGLISH, LANGUAGE)
      if (bucket === 'guide' && GUIDE_LANG_IDS.includes(id)) {
        const nonLangSelected = current.filter((x) => !GUIDE_LANG_IDS.includes(x));
        const langSelected = current.includes(id) ? [] : [id];
        map[dayNum] = [...nonLangSelected, ...langSelected];
      } else {
        // ✅ Multi-Select for Transport, Entrances, Activities, Misc, and all other guides
