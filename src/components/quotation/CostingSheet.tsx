...
const gross = (
  ...
  const withMk = net * (1 + mk / 100);
  return withMk * (1 + gst / 100);
};
...
function OptionCell({
  ...
  if (opts.length === 0) {
    return (
      <span className="block p-1.5 text-right text-muted-foreground">
        —
      </span>
    );
  }
...
function RateSheetBlock({
  ...
  const exportRows: RateSheetExportRow[] = exportGroups.flatMap((g) =>
    g.rows.map((r) => {
      // buildRateSheet is the single source of truth for exact/range pax
      // repricing, including activity slabs and per-person rules.
      const activities = r.activities;
      const common =
        r.transport + r.guide + r.escort +
        r.entrances + activities + r.misc + r.lunch + r.dinner;
      return {
        vehicle: g.vehicle,
        pax: r.pax,
  ...
  return (
    <>
      ...
      {g.rows.map((r) => {
         ...
         * Rate Sheet row.
         */
        const activitiesPerPerson = r.activities;
