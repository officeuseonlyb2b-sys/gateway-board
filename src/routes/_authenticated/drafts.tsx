...
const STEP_LABELS = [
  "Type", "Who", "Duration", "Program", "Dates+Pax", "Category",
  "From", "Travel", "Routing", "Transport", "Activities", "Entrances",
  "Guide", "Misc", "Hotels", "Costing", "Final", "Optionals",
];
...
function DraftsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return drafts.filter((d) =>
      (d.draft_name + " " + (d.wizard_state.query_type || "") + " " + d.wizard_state.program_name).toLowerCase().includes(needle),
    );
  }, [drafts, q]);
  ...
  function beginRename(d: DraftRecord) { setEditingId(d.id); setEditName(d.draft_name); }
  function saveRename() {
    ...
    <div className="p-6 lg:p-8 max-w-[1500px] mx-auto">
      ...
      <Card className="p-4 mb-4">
        <div className="relative">
          ...
          <Input placeholder="Search drafts by name, program, or type…" value={q}
      ...
      <Card className="p-0 overflow-hidden">
        {filtered.length === 0 ? (
          <div className="py-16 text-center text-sm text-muted-foreground">
            ...
            No drafts yet. Start a New Quotation and use "Save Draft" to keep it here.
          </div>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((d) => {
                const stepLabel = STEP_LABELS[Math.max(0, Math.min(17, d.last_step - 1))];
                return (
                  <tr key={d.id} className="border-b last:border-0 hover:bg-muted/20">
                    <td className="py-2.5 px-4 font-medium">
                      {editingId === d.id ? (
                        <div className="flex items-center gap-1">
                          ...
                          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={saveRename}><Check className="h-3.5 w-3.5" /></Button>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button className="opacity-0 group-hover:opacity-100" onClick={() => beginRename(d)}>
                    ...
                    <td className="py-2.5 px-2">
                      {d.wizard_state.query_type && <Badge variant="secondary">{d.wizard_state.query_type}</Badge>}
                    </td>
                    <td className="py-2.5 px-2 text-muted-foreground">{d.wizard_state.program_name || "—"}</td>
                    <td className="py-2.5 px-2 text-xs">
                      <span className="tabular-nums">{d.last_step}/18</span>
                      <span className="text-muted-foreground ml-1.5">{stepLabel}</span>
                    </td>
                    <td className="py-2.5 px-2 text-muted-foreground text-xs">{fmtDateShort(d.updated_at)}</td>
                    <td className="py-2.5 px-4">
                      <div className="flex justify-end gap-1">
                        <Button size="sm" variant="outline" onClick={() => resume(d)}>
                          <Play className="h-3.5 w-3.5 mr-1" /> Resume
                        </Button>
                        <Button size="icon" variant="ghost" title="Rename" onClick={() => beginRename(d)}>
                          <Pencil className="h-4 w-4" />
                        </Button>
                        <Button size="icon" variant="ghost" title="Delete"
                          onClick={() => {
                            if (confirm(`Delete draft "${d.draft_name}"?`)) {
                              deleteDraft(d.id); toast.success("Draft deleted.");
                            }
                          }}>
