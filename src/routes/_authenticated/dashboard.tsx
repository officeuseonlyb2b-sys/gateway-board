...
import {
  ...
  ArrowUpRight, Plus, Calculator, Search, Upload, Eye, Printer, Download,
} from "lucide-react";
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-[1600px] mx-auto">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-3">Overview of your hotel portfolio and rate operations.</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        <StatCard icon={Hotel} label="Total Hotels" value={stats.hotels} trend={stats.updatedThisMonth > 0 ? `+${stats.updatedThisMonth} this month` : undefined} />
        ...
        <StatCard icon={FileText} label="Saved Quotes" value={stats.savedQuotes} active />
      </div>
        ...
        <Card className="xl:col-span-3 p-6 card-elevated border-0">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">Latest rate or hotel detail changes.</p>
            </div>
          ...
          <div className="overflow-x-auto -mx-6 px-6">
            <table className="w-full text-sm">
              ...
              <tbody>
                {recent.map((h) => (
                  <tr key={h.id} className="border-b border-border transition-colors hover:bg-[#F9FAFB]">
                    ...
                    <td className="py-3 pr-4">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-medium border ${CATEGORY_BADGE[h.hotel_category]}`}>
                        {h.hotel_category}
                      </span>
                    </td>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 8, right: 4, left: -18, bottom: 0 }}>
                ...
                <Bar dataKey="count" radius={[6, 6, 0, 0]} fill="#1A4A6B">
                  {chartData.map((_, i) => <Cell key={i} fill="#1A4A6B" />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        <Card className="p-6 card-elevated border-0">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">Seasons covered per hotel (target: 4).</p>
            </div>
          </div>
              ...
              return (
                <div key={c.id}>
                  <div className="flex items-center justify-between text-sm mb-1.5">
                    <Link to="/hotels/$id" params={{ id: c.id }} className="font-medium text-primary hover:text-accent truncate flex items-center gap-2">
      ...
      <Card className="p-6 card-elevated border-0">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="section-label">Recent Saved Quotes</h2>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {recentQuotes.map((q) => (
                <tr key={q.id} className="border-b border-border hover:bg-[#F9FAFB]">
                  <td className="py-3 pl-3 pr-4 font-mono text-xs">
                    <Link to="/quotes" className="hover:text-accent font-semibold text-primary">{q.quote_number}</Link>
                  </td>
                  <td className="py-3 pr-4 text-primary">{q.cities.join(" → ") || q.tour_title}</td>
                  <td className="py-3 pr-4 tabular-nums">{q.total_nights}</td>
                  <td className="py-3 pr-4 text-right tabular-nums font-semibold text-primary">₹{q.totals.grand_dbl.toLocaleString("en-IN")}</td>
                  <td className="py-3 pr-4 text-muted-foreground text-xs">{new Date(q.saved_at).toLocaleDateString("en-IN")}</td>
                  <td className="py-3 pr-4 text-right">
                    <div className="inline-flex gap-1">
                      <Link to="/quotes" className="h-7 w-7 rounded-md hover:bg-[#F3F4F6] inline-flex items-center justify-center text-muted-foreground hover:text-primary" title="View">
                        <Eye className="h-3.5 w-3.5" />
                      </Link>
                      <Link to="/quotes" className="h-7 w-7 rounded-md hover:bg-[#F3F4F6] inline-flex items-center justify-center text-muted-foreground hover:text-primary" title="Print">
                        <Printer className="h-3.5 w-3.5" />
                      </Link>
                      <Link to="/quotes" className="h-7 w-7 rounded-md hover:bg-[#F3F4F6] inline-flex items-center justify-center text-muted-foreground hover:text-primary" title="Download">
              ...
              {recentQuotes.length === 0 && (
                <tr><td colSpan={6} className="py-8 text-center text-muted-foreground text-sm">No saved quotes yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function StatCard({
  icon: Icon, label, value, trend, active,
}: { icon: typeof Hotel; label: string; value: number; trend?: string; active?: boolean }) {
  return (
    <Card
...
function QuickAction({
  to, icon: Icon, label, variant,
}: { to: string; icon: typeof Plus; label: string; variant?: "primary" | "accent" }) {
  const cls =
    variant === "accent"
