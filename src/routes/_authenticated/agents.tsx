...
import {
  useAgents, deleteAgent, type Agent,
...
function AgentsPage() {
  ...
  const states = useMemo(() => Array.from(new Set(agents.map((a) => a.state).filter(Boolean))) as string[], [agents]);
  const cities = useMemo(() => Array.from(new Set(agents.map((a) => a.city).filter(Boolean))) as string[], [agents]);
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-[1400px] mx-auto">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><Users className="h-6 w-6 text-accent" /> Agents</h1>
          <p className="text-sm text-muted-foreground">Master directory of travel agents used across quotations.</p>
        </div>
        <Button onClick={openNew}><Plus className="h-4 w-4 mr-1" /> Add Agent</Button>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          <div className="md:col-span-2">
            ...
            <div className="relative">
              ...
              <Input className="pl-8" placeholder="Name, agency, mobile, email, city, GST…" value={q} onChange={(e) => { setQ(e.target.value); setPage(1); }} />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <Select value={stateF} onValueChange={(v) => { setStateF(v); setPage(1); }}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All</SelectItem>
          ...
          <div>
            ...
            <Select value={cityF} onValueChange={(v) => { setCityF(v); setPage(1); }}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All</SelectItem>
      ...
      <Card>
        <Table>
          ...
          <TableBody>
            {paged.length === 0 ? (
              <TableRow><TableCell colSpan={7} className="text-center py-10 text-muted-foreground">No agents found</TableCell></TableRow>
            ) : paged.map((a) => (
              <TableRow key={a.id}>
                <TableCell>
                  <div className="font-medium">{a.name}</div>
                  {a.contact_person && a.contact_person !== a.name && <div className="text-xs text-muted-foreground">{a.contact_person}</div>}
                </TableCell>
                ...
                <TableCell className="text-sm">{[a.city, a.state].filter(Boolean).join(", ") || "—"}</TableCell>
                <TableCell className="text-xs">{a.gst_number || "—"}</TableCell>
                <TableCell>
                  <Badge variant={(a.status || "Active") === "Active" ? "default" : "secondary"}>{a.status || "Active"}</Badge>
                </TableCell>
                <TableCell className="text-right">
                  <div className="inline-flex gap-1">
                    ...
                    <Button size="icon" variant="ghost" onClick={() => setConfirmDel(a)} title="Delete"><Trash2 className="h-4 w-4 text-destructive" /></Button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between p-3 border-t text-sm">
            ...
            <div className="flex gap-2">
              <Button size="sm" variant="outline" disabled={pageSafe <= 1} onClick={() => setPage(pageSafe - 1)}>Previous</Button>
              <Button size="sm" variant="outline" disabled={pageSafe >= totalPages} onClick={() => setPage(pageSafe + 1)}>Next</Button>
            </div>
          </div>
        )}
      </Card>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {confirmDel && <>This will remove <b>{confirmDel.name} — {confirmDel.agency}</b> from the master. Existing quotations already using this agent will keep their stored details.</>}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="pt-3 border-t text-xs text-muted-foreground">
        Created {a.created_at ? new Date(a.created_at).toLocaleString() : "—"} · Updated {a.updated_at ? new Date(a.updated_at).toLocaleString() : "—"}
      </section>
    </div>
  );
}
