...
export default function NewQuery() {
  ...
  const [selectedAgentContactId, setSelectedAgentContactId] = useState("");
  ...
  const selectedProgramme = programmes.find((program) => program.id === selectedProgramId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isB2B && !selectedAgentContactId) {
      toast.error("Select the person from the Agent company who sent this Query.");
      return;
    }
    ...
    try {
      ...
      createLead({
        ...
        pax,
        destination: selectedProgramme?.name || tourStartCity || queryBaseCity || tourEndCity,
        priority: "Normal",
        requirement: queryFor,
        created_by: user?.name || "Sales Desk",
        customer_type: isB2B ? "B2B Agent" : "B2C Client",
        agent_id: matchedAgent?.id,
        agent_contact_id: isB2B ? selectedAgentContactId : undefined,
        ...
        program_id: selectedProgramme?.id,
        program_name: selectedProgramme?.name,
      ...
      try {
        ...
        console.error("Query created locally but cloud sync failed", syncError);
        toast.warning("Query created locally, but cloud sync failed.");
      }
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <>
                      <Select
                        ...
                        onValueChange={(value) => {
                          ...
                          const contact =
                            agent.contacts?.find((item) => item.id === agent.primary_contact_id) ||
                            agent.contacts?.find((item) => item.active !== false) ||
                            agent.contacts?.[0];
                          setSelectedAgentContactId(contact?.id || "");
                          setContactPerson(contact?.name || agent.contact_person || agent.name);
                          setContactNumber(contact?.phone || agent.phone || "");
                          setEmailId(contact?.email || agent.email || "");
                ...
                <div className="space-y-2">
                  ...
                  {isB2B ? (
                    <Select
                      value={selectedAgentContactId}
                      onValueChange={(value) => {
                        setSelectedAgentContactId(value);
                        const agent = agents.find((item) => item.id === selectedAgentId);
                        const contact = agent?.contacts?.find((item) => item.id === value);
                        if (!contact) return;
                    ...
                    >
                      <SelectTrigger>
                        <SelectValue
                          placeholder={
                            selectedAgentId ? "Select company contact" : "Select an Agent first"
                          }
                        />
                      </SelectTrigger>
                      <SelectContent>
                        {(agents.find((item) => item.id === selectedAgentId)?.contacts || [])
                      ...
                      {programmes
                        ...
                        .map((program) => (
                          <SelectItem key={program.id} value={program.id}>
                            {program.code} · {program.name} · {program.nights}N/{program.days}D
                          </SelectItem>
                        ))}
                    </SelectContent>
                  </Select>
