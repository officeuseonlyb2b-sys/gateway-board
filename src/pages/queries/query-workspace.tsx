...
import { quoteToRevisionDraft } from "@/lib/quotes-store";
import { upsertDraft, useDrafts } from "@/lib/drafts-store";
...
export default function QueryWorkspace() {
  ...
  const linkedDrafts = allDrafts
    .filter((draft) => draft.wizard_state.linked_query_id === query?.query_id)
    ...
    <div className="min-h-screen bg-slate-50 p-6 lg:p-8">
      <div className="mx-auto max-w-[1600px] space-y-6">
        ...
        <div className="grid gap-3 rounded-xl border bg-white p-4 md:grid-cols-5">
          <Fact label="Travellers">{paxRangeLabel(query)}</Fact>
              ...
              <Card>
                ...
                <CardContent className="grid gap-3 md:grid-cols-2">
                  ...
                  <Fact label="Contact">
                    {query.agent_contact_id ? (
                      <Link
                        ...
                        params={{ id: query.agent_contact_id }}
                        className="font-semibold text-teal-700 hover:underline"
                      >
                  ...
                  <Fact label="Assignment">
                    ...
                    <br />
                    Assigned by {query.assigned_by || "—"} · Owner {query.owner || "Pending"}
                  </Fact>
                  <Fact label="Commercial ownership">
                    Costing: {query.last_costing_prepared_by || "Not recorded"}
                    <br />
                    Quotation: {query.last_quotation_prepared_by || "Not recorded"}
                  </Fact>
                  <Fact label="Latest action ownership">
                    Follow-up: {query.last_followup_by || "Not recorded"}
                    <br />
                    Stage change: {query.last_status_changed_by || "Not recorded"}
                  </Fact>
                </CardContent>
              </Card>
              <LifecycleClock />
          ...
          <div className="space-y-5">
            ...
            {linkedDrafts.length > 0 && (
              <Card className="border-amber-200 bg-amber-50/40">
                <CardHeader>
                  <CardTitle className="text-base">Quotation drafts in progress</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2">
                  {linkedDrafts.map((draftRecord) => (
                    <div
                      key={draftRecord.id}
                      className="flex flex-col justify-between gap-3 rounded-lg border bg-white p-3 sm:flex-row sm:items-center"
                    >
                      <div>
                        <p className="font-semibold">{draftRecord.draft_name}</p>
                        <p className="text-xs text-slate-500">
                          Step {draftRecord.last_step}/10 · Updated {fmt(draftRecord.updated_at)}
                        </p>
                      </div>
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => navigate({ to: "/costing", search: { id: draftRecord.id } })}
                      >
                        Resume draft
                      </Button>
                    </div>
                    ...
                    <Button
                      onClick={() => {
                        const reason = window.prompt(
                          `Reason for revising ${version.quote.display_number || version.quote.quote_number}:`,
                        );
                        if (!reason?.trim()) return;
                        try {
                          const revision = quoteToRevisionDraft(version.quote, reason);
                          const draftId = upsertDraft(
                            revision,
                            undefined,
                            `${version.quote.quote_number} · Revision V${revision.intended_version}`,
                          );
                          writeDraft(revision);
