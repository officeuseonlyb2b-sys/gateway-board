...
export interface CrmMetrics {
  ...
  monthly: {
    ...
    prevQuotesSent: number;
    prevConfirmed: number;
    prevLost: number;
    prevNurturing: number;
...
export function computeMetrics(
  ...
  const dueOn = (day: Date, list = queries) =>
    list.filter((q) => isOpen(q) && sameDay(q.followup_due, day)).length;
  const overdueList = (list = queries) =>
    list.filter((q) => isOpen(q) && new Date(q.followup_due).getTime() < today.getTime());

  const pendingQuotations = queries.filter(
    (q) => q.stage === "Costing" || q.stage === "Quotation Sent",
  ).length;
  ...
  const workload: WorkloadRow[] = roster
    ...
    .map((employee) => {
      ...
      const confirmed = mine.filter((q) => q.stage === "Won").length;
      const closed = mine.filter((q) => q.stage === "Won" || q.stage === "Lost").length;
      return {
        employee,
        assigned: mineActive.length,
        newToday: mine.filter((q) => sameDay(q.created_at, today)).length,
        inProgress: mine.filter((q) => q.stage === "Requirement Review" || q.stage === "Costing")
          .length,
        ...
        revenue:
          mine.filter((q) => q.stage === "Won").reduce((s, q) => s + q.value, 0) ||
  ...
  const monthStats = (m: number) => {
    ...
    const confirmed = list.filter((q) => q.stage === "Won").length;
    return {
      total: list.length,
      quotes: list.filter((q) => ["Quotation Sent", "Follow-up", "Won", "Lost"].includes(q.stage))
        .length,
      confirmed,
  ...
  const partners = [...partnerMap.entries()]
    .map(([name, revenue]) => ({
      name,
      revenue,
      share: partnerTotal ? (revenue / partnerTotal) * 100 : 0,
    }))
  ...
  const stageAveragesByEmployee = new Map<
    string,
    { stage: Stage; avgHours: number; samples: number }[]
  >();
  roster.forEach((emp) => {
  ...
  const respSamples = stageEvents
    .filter((e) => e.from_stage === "New")
    .map((e) => e.duration_hours!);
  ...
  const trends = weeks.map(({ key, label, start, end }) => {
    ...
    const byEmployee: Record<
      string,
      { leads: number; won: number; lost: number; conversion: number }
    > = {};
    roster.forEach((emp) => {
  ...
  const criticalEscalations = escalations.filter((e) => e.overdueHours >= 48);

  const closedAll = queries.filter((q) => q.stage === "Won" || q.stage === "Lost").length;
  const conversion = queries.length
    ? (queries.filter((q) => q.stage === "Won").length / queries.length) * 100
    : 0;
  ...
  return {
    ...
    prev: {
      ...
      overdue: queries.filter(
        (q) => isOpen(q) && new Date(q.followup_due).getTime() < yesterday.getTime(),
      ).length,
      pendingQuotations,
    },
    workload,
    ...
    monthly: {
      label: monthName(0),
      prevLabel: monthName(-1),
      totalLeads: cur.total,
      quotesSent: cur.quotes,
      confirmed: cur.confirmed,
      lost: cur.lost,
      nurturing: cur.nurturing,
      conversion: cur.conversion,
      prevTotalLeads: prevM.total,
      prevQuotesSent: prevM.quotes,
      prevConfirmed: prevM.confirmed,
      prevLost: prevM.lost,
      prevNurturing: prevM.nurturing,
      prevConversion: prevM.conversion,
    },
    partners,
    weekly: {
      ...
      leadsPendingAction: queries.filter(
        (q) => q.stage === "New" || q.stage === "Requirement Review",
      ).length,
    },
...
export function useCrmMetrics(): CrmMetrics {
  ...
  return useMemo(
    () => computeMetrics(queries, tasks, employees, events),
...
export function usePersonalMetrics(ownerName: string) {
  ...
  return useMemo(() => {
    ...
    return {
      ...
      myTasks,
      todaysTasks: myTasks
        .filter((t) => !t.done && new Date(t.due_at).getTime() <= today.getTime() + 864e5)
        .sort((a, b) => (a.due_at < b.due_at ? -1 : 1)),
      newAssigned: mine.filter((q) => q.stage === "New").length,
      quotationsPending: mine.filter((q) => q.stage === "Costing" || q.stage === "Quotation Sent")
        .length,
      followupsDueToday: mine.filter((q) => isOpen(q) && sameDay(q.followup_due, today)).length,
      overdueTasks: myTasks.filter((t) => !t.done && new Date(t.due_at).getTime() < today.getTime())
        .length,
      tasksCompletedToday: m.events.filter(
        (e) => e.type === "task_completed" && e.by === ownerName && sameDay(e.at, today),
      ).length,
      pipeline: (
        [
          "New",
