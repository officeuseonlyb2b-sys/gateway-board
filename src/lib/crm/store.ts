...
const EMPLOYEE_ROSTER_KEY = "mp_crm_employee_roster_revision";
const EMPLOYEE_ROSTER_REVISION = "mp-sales-roster-2026-09-18-v1";
...
const APPROVED_EMPLOYEE_ROSTER: Array<
  Pick<
    Employee,
    "id" | "name" | "role" | "designation" | "department" | "data_scope" | "dashboard_template"
  > & {
    permissions: string[];
  }
> = [
  {
    id: "emp_roster_ashish_prajapati",
    name: "Ashish Prajapati",
    role: "Owner / Director",
    designation: "CEO",
    department: "Management",
    data_scope: "Organisation",
    dashboard_template: "Sales Control Tower",
    permissions: [
      "query.create",
      "query.read",
      "query.work",
  ...
  {
    id: "emp_roster_ramesh_rotele",
    name: "Ramesh K Rotele",
    role: "Vendor Executive",
    designation: "Vendor Manager",
    department: "Vendor Management",
    data_scope: "Unit",
    dashboard_template: "My Sales Desk",
    permissions: ["query.create", "query.read", "vendor.manage"],
  },
  {
    id: "emp_roster_shivam_kushwah",
    name: "Shivam Kushwah",
    role: "Operations Executive",
    designation: "Senior Executive - Tour Operations",
    department: "Operations",
    data_scope: "Unit",
    dashboard_template: "Operations Desk",
    permissions: ["query.create", "query.read", "operations.work"],
  },
  {
    id: "emp_roster_deeksha_saini",
    name: "Deeksha Saini",
    role: "Sales Manager",
    designation: "Manager - Tour Sales",
    department: "Sales",
    data_scope: "Unit",
    dashboard_template: "Sales Control Tower",
    permissions: [
  ...
  {
    id: "emp_roster_chhaya_prajapati",
    name: "Chhaya Prajapati",
    role: "Sales Executive",
    designation: "Senior Executive - Tour Sales",
    department: "Sales",
    data_scope: "Own",
    dashboard_template: "My Sales Desk",
    permissions: [
  ...
  {
    id: "emp_roster_bhumika_prajapati",
    name: "Bhumika Prajapati",
    role: "Sales Executive",
    designation: "Executive - Tour Sales",
    department: "Sales",
    data_scope: "Own",
    dashboard_template: "My Sales Desk",
    permissions: [
...
const employeeNameKey = (name?: string) => (name || "").trim().toLowerCase().replace(/\s+/g, " ");

function ensureApprovedEmployeeRoster(
  existing: Employee[],
  applyApprovedDefaults = false,
): Employee[] {
  const remaining = [...existing];
  const now = iso(new Date());
  const approved = APPROVED_EMPLOYEE_ROSTER.map((template, index) => {
    const matchIndex = remaining.findIndex(
      (employee) => employeeNameKey(employee.name) === employeeNameKey(template.name),
    );
    const current = matchIndex >= 0 ? remaining.splice(matchIndex, 1)[0] : undefined;
    const applyTemplate =
      applyApprovedDefaults || current?.roster_revision !== EMPLOYEE_ROSTER_REVISION;
    const placeholderEmail = `pending+${template.name
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, ".")
      .replace(/^\.|\.$/g, "")}@employee.local`;
    return {
      ...template,
      ...(applyTemplate ? current : { ...template, ...current }),
      ...(applyTemplate ? template : {}),
      id: current?.id || template.id,
      email:
        current?.email && !current.email.endsWith("@employee.local")
          ? current.email
          : placeholderEmail,
      target_monthly: current?.target_monthly ?? 0,
      active: current?.active ?? true,
      joined_at: current?.joined_at || now,
      employee_code: current?.employee_code || `EMP-${String(index + 1).padStart(4, "0")}`,
      employee_status: current?.employee_status || "Active",
      account_status: current?.account_status || "Active",
      unit: current?.unit || "Madhya Pradesh",
      unit_scope: current?.unit_scope?.length ? current.unit_scope : ["Madhya Pradesh"],
      permission_grants: current?.permission_grants || [],
      permission_restrictions: current?.permission_restrictions || [],
      login_email_pending: !(current?.email && !current.email.endsWith("@employee.local")),
      roster_revision: EMPLOYEE_ROSTER_REVISION,
    } satisfies Employee;
  });
  const ashishId = approved.find((employee) => employee.name === "Ashish Prajapati")?.id;
  const deekshaId = approved.find((employee) => employee.name === "Deeksha Saini")?.id;
  return [
    ...approved.map((employee) => ({
      ...employee,
      manager_id:
        employee.name === "Deeksha Saini"
          ? ashishId
          : ["Chhaya Prajapati", "Bhumika Prajapati"].includes(employee.name)
            ? deekshaId
            : employee.manager_id,
    })),
    ...remaining,
...
function load() {
  ...
  const applyRosterDefaults =
    localStorage.getItem(EMPLOYEE_ROSTER_KEY) !== EMPLOYEE_ROSTER_REVISION;
  employees = ensureApprovedEmployeeRoster(
    parse<Employee>(localStorage.getItem(EMP_KEY)),
    applyRosterDefaults,
  );
  localStorage.setItem(EMP_KEY, JSON.stringify(employees));
  localStorage.setItem(EMPLOYEE_ROSTER_KEY, EMPLOYEE_ROSTER_REVISION);
...
export function hydrateCrm(snapshot: CrmSnapshot) {
  ...
  employees = ensureApprovedEmployeeRoster(snapshot.employees);
...
export function addEmployee(input: EmployeeInput): Employee {
  ...
  const emp: Employee = {
    ...
    login_email_pending: input.login_email_pending ?? input.email.endsWith("@employee.local"),
  };
...
export function setQueryStage(
  ...
  queries = queries.map((q) => {
    ...
    return {
      ...
      last_quotation_sent_by: stage === "Quotation Sent" ? actor : q.last_quotation_sent_by,
...
export function saveQueryCosting(
  ...
  queries = queries.map((q) => {
    ...
    if (previous.some((item) => item.quote.id === quote.id)) return q;
    const version = quote.version || previous.reduce((max, item) => Math.max(max, item.version), 0) + 1;
    const scenarioValues = (quote.scenarios ?? [])
      .map((scenario) => scenario.grand_total)
      .filter((value) => Number.isFinite(value) && value > 0);
    const sellingPrice = scenarioValues.length
      ? Math.max(...scenarioValues)
      : Math.max(quote.totals.grand_sgl, quote.totals.grand_dbl, quote.totals.grand_trp);
    const scenarioCosts = (quote.scenarios ?? []).map((scenario) =>
      scenario.persons.reduce(
        (sum, person) => sum + person.hotel + person.transport + person.guide
          + person.activities + person.entrances + person.misc + person.meals,
        0,
      ),
    );
    const costPrice = scenarioCosts.length
      ? Math.max(...scenarioCosts)
      : Math.max(quote.totals.room_net_sgl, quote.totals.room_net_dbl, quote.totals.room_net_trp)
        + quote.totals.addons_total;
    const bottomLine =
      quote.query_snapshot?.bottom_line ||
      (scenarioValues.length ? Math.min(...scenarioValues) : sellingPrice);
    const topLine =
      quote.query_snapshot?.top_line ||
    ...
    const categoryRank = (value: string) => {
      ...
      if (stars)
        return (
          stars * 10 + (normalized.includes("deluxe") || normalized.includes("superior") ? 1 : 0)
        );
    ...
    const scenarioPax = (quote.scenarios ?? [])
      ...
      .filter((value) => Number.isFinite(value) && value > 0);
    const minPax =
      quote.query_snapshot?.pax_min ||
      (scenarioPax.length ? Math.min(...scenarioPax) : q.min_pax || q.pax);
    const maxPax =
      quote.query_snapshot?.pax_max ||
    ...
    return {
      ...
      last_costing_prepared_by: by,
      last_quotation_prepared_by: by,
      value: topLine,
      pax: maxPax || q.pax,
      ...
      costing_versions: [
        ...previous,
        {
          ...
          saved_by: by,
          // A generated version is immutable. `draftId` is intentionally not
          // retained as an edit link; revisions are cloned from quote snapshot.
          draft_id: undefined,
          quote: { ...quote, version },
        },
      ],
