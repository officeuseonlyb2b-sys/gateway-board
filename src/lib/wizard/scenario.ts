...
export function computeScenario(
  ...
  const includesAccommodation = commercialMode !== "transport_only";
  const includesLand = commercialMode === "package";
  const includesTransportOnly = commercialMode === "transport_only";
  ...
  if (includesAccommodation) draft.routing.forEach((day, i) => {
  ...
  // Supplements remain outside the package total.  They are shown for
  // reference in the optional-supplement step but are never silently added.
  const addons_total = transport_total + guide_total + activities_total
    + entrances_total + misc_total + meals_total;

  // Land Part and Hotels & Meals carry their own markup / GST percentages.
  const landMk = includesLand || includesTransportOnly ? landMarkup(draft) : 0;
  ...
  return {
    ...
    option_key: opt.key,
    option_label: includesAccommodation
      ? opt.category || opt.label || `Option ${opt.key}`
      : "Transport only",
